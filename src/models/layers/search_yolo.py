...
class YOLOBottleneck_search(SearchModule):
    # Standard bottleneck
    def __init__(self, in_channel, out_channel, candidate_op=[(3,1),(5,1),(3,2)], candidate_ch=[1.], shortcut=True, group=1, expansion=0.5, gumbel_op=False, gumbel_channel=False, separable=False, merge_kernel=True, with_arch_param=True):  
        ...
        self.cv1 = ConvBNAct_search(in_channel, c_max, candidate_op=[(1,1)], candidate_ch=candidate_ch, stride=1, gumbel_op=gumbel_op, gumbel_channel=gumbel_channel, act=nn.SiLU, bn=nn.BatchNorm2d, merge_kernel=merge_kernel, independent_op_arch_param=with_arch_param, independent_ch_arch_param=with_arch_param)
        if separable: my_conv = SepConvBNAct_search
        else: my_conv = ConvBNAct_search
        self.cv2 = my_conv(c_max, out_channel, candidate_op, candidate_ch=[1.], stride=1, group=group, gumbel_op=gumbel_op, gumbel_channel=gumbel_channel, act=nn.SiLU, bn=nn.BatchNorm2d, merge_kernel=merge_kernel, independent_op_arch_param=with_arch_param, independent_ch_arch_param=with_arch_param)
...
class YOLOC3_search(SearchModule):
    ...
    def __init__(self, in_channel, out_channel, num_repeat=1, candidate_op=[(3,1),(5,1),(3,2)], candidate_ch=[1.], shortcut=True, group=1, expansion=0.5, e_bottleneck=1., search_out_channel=None, gumbel_op=False, gumbel_channel=False, separable=False, merge_kernel=True, with_arch_param=True):  
        ...
        c_ = int(out_channel * expansion)  # hidden channels
        self.cv1 = ConvBNAct_search(in_channel, c_, candidate_op=[(1,1)], candidate_ch=self.search_out_channel, stride=1, gumbel_op=gumbel_op, gumbel_channel=gumbel_channel, independent_ch_arch_param=False, independent_op_arch_param=with_arch_param, merge_kernel=merge_kernel, bn=nn.BatchNorm2d, act=nn.SiLU)
        self.cv2 = ConvBNAct_search(in_channel, c_, candidate_op=[(1,1)], candidate_ch=self.search_out_channel, stride=1, gumbel_op=gumbel_op, gumbel_channel=gumbel_channel, independent_ch_arch_param=False, independent_op_arch_param=with_arch_param, merge_kernel=merge_kernel, bn=nn.BatchNorm2d, act=nn.SiLU)

        self.gumbel_channel = gumbel_channel and len(self.search_out_channel) > 1
        self.cv3 = nn.ModuleList([ConvBNAct_search(c_, out_channel, candidate_op=[(1,1)], candidate_ch=self.search_out_channel, stride=1, gumbel_op=gumbel_op, gumbel_channel=gumbel_channel, act=False, bn=False, independent_ch_arch_param=False, independent_op_arch_param=with_arch_param) for _ in range(2)])  
        self.cv3_act = nn.SiLU()
        self.cv3_bn = nn.ModuleList([nn.BatchNorm2d(int(out_channel*e)) for e in self.search_out_channel])
#        else:
#            self.cv3 = ConvBNAct_search(2 * c_, out_channel, candidate_op=[(1,1)], candidate_ch=self.search_out_channel, stride=1, gumbel_op=gumbel_op, gumbel_channel=gumbel_channel, independent_ch_arch_param=False, independent_op_arch_param=with_arch_param, merge_kernel=merge_kernel, bn=nn.BatchNorm2d, act=nn.SiLU())  

        if len(self.search_out_channel)>1 and with_arch_param:
            self.init_arch_parameters('ch_alphas', len(self.search_out_channel))
        else: self.ch_alphas = torch.tensor([1.]*len(self.search_out_channel))

        self.m = nn.Sequential(*[YOLOBottleneck_search(c_, c_, candidate_op, candidate_ch, shortcut, group, expansion=self.e_bottleneck[i], gumbel_op=gumbel_op, gumbel_channel=gumbel_channel, separable=separable, merge_kernel=merge_kernel, with_arch_param=with_arch_param) for i in range(num_repeat)])

    def forward(self, x):
        ch_alphas = self.norm_arch_parameters(self.ch_alphas, self.gumbel_channel) if len(self.ch_alphas)>1 else self.ch_alphas
#        if self.gumbel_channel:
