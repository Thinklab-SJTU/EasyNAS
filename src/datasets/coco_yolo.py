...
from .coco_yolo_utils import xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, resample_segments, \
    clean_str
from app.distribute_utils import torch_distributed_zero_first
...
#    nw = min([os.cpu_count() // world_size, batch_size if batch_size > 1 else 0, workers])  # number of workers
    nw = min([os.cpu_count() // world_size, workers])  # number of workers
...
def create_dataloader_search(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', train_portion=0.5):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, imgsz, batch_size,
                                      augment=augment,  # augment images
    ...
    dataset_train = torch.utils.data.Subset(dataset, indices=indices[:split])
    dataset_val = torch.utils.data.Subset(dataset, indices=indices[split:])
  
#    nw = min([os.cpu_count() // world_size, batch_size if batch_size > 1 else 0, workers])  # number of workers
    nw = min([os.cpu_count() // world_size, workers])  # number of workers
    loader = torch.utils.data.DataLoader if image_weights else InfiniteDataLoader
    # Use torch.utils.data.DataLoader() if dataset.properties will update during training else InfiniteDataLoader()
    sampler = torch.utils.data.distributed.DistributedSampler(dataset_train) if rank != -1 else None
...
#                        sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[:split]),
    ...
    sampler = torch.utils.data.distributed.DistributedSampler(dataset_val) if rank != -1 else None
...
def img2label_paths(img_paths):
#    return ['txt'.join(x.replace("coco2017/images", "ShareData/koutengchuan/yolov5-master/data/coco/labels", 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
    return ['txt'.join(x.replace("/images/", "/labels/", 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
...
def load_mosaic(self, index):
    ...
    # Augment
    img4, labels4, segments4 = copy_paste(img4, labels4, segments4, probability=self.hyp.get('copy_paste', 0))
...
def load_mosaic9(self, index):
    ...
    # Augment
    img9, labels9, segments9 = copy_paste(img9, labels9, segments9, probability=self.hyp.get('copy_paste', 0))
